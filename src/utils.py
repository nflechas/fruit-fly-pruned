...
def read_space(dm_file:str) -> Union[np.array, list]: 
    """
    parameters:
    -------
        dm_file: path to .dm file
...
##########################################
# Pruned experiments utils

def load_embeddings(path_data: Path, matrix_type: str) -> np.array:
    """
    Loads embeddings matrices.

    parameters:
        - matrix_type: str. Etiher "pruned" or "unpruned"
    
    output:
        - embeddings_matrices: np.array of embeddings.
    """
    if matrix_type == "pruned":
        embeddings_matrices = np.load(path_data / "pruned_dnn_mat_pca_analysis_Spearman.npz")
    elif matrix_type == "unpruned":
        embeddings_matrices = np.load(path_data / "glove_data.npz")
    
    return embeddings_matrices

def read_glove_space(path_glove_space: Path) -> Union[np.array, list]: 
    """
    parameters:
    -------
        path_glove_space: path to .dm file
    ...
    #     vec=[float(i) for i in items[1:]]
    #     m.append(vec)
    glove_dict = np.load(path_glove_space)
    m = np.array(list(glove_dict.values()))
    vocab = list(glove_dict.keys())

    return m, vocab

def prune_m(m:np.array, prune_cat_path:Path, num_common_cat:int) -> np.array:
    """
    Paramaters:
    ----------
    prune_cat_path: path to csv file which states which categories preserved \\
        which features ater pruning.
    num_common_cat: at least how many categories must have retained a feature \\
        for it to be preserved.
    
    Returns:
    ----------
    pruned_m: pruned np.array.
    """
    print(prune_cat_path)
    retained_feats = pd.read_csv(prune_cat_path)
    filter = [True if i >= num_common_cat else False for i in retained_feats["sum"]]
    pruned_m = np.array(list(vec[filter] for vec in m))
