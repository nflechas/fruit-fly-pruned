...
Usage:
  run_fly.py --matrix_type=<str>
  run_fly.py --matrix_type=<str> --num_cat=<n>
  run_fly.py (-h | --help)
  run_fly.py --version
...
Options:
  ...
  --matrix_type=<str>          "pruned" or "unpruned".
  --num_cat=<n>  
"""
...
from utils import read_space, hash_dataset_, read_cols, read_glove_space, prune_m
from pathlib import Path
import sys
sys.path.append("/workspaces/fruit-fly-pruned")
import src.MEN as MEN
import numpy as np
...
class Fly():
    ...
    def similarities(self,m=None,metric='cosine'):
        ...
        if metric == 'hamming':
            similarities = 1-pairwise_distances(np.asarray(m.todense()), metric="hamming")
        return similarities
...
if __name__ == '__main__':
    ...
    # dataset = "wiki"#args["--dataset"]
    matrix_type = args["--matrix_type"]
    num_common_cat = int(args["--num_cat"]) #
    ...
    MEN_annot = Path.cwd().parent / "data/MEN_dataset_natural_form_full"
    # Load space
    path_glove = Path.cwd().parent / "data/MEN_glove_data.npz"
    m_original, vocab = read_glove_space(path_glove)

    if matrix_type == "pruned":
    # prune embeddings
        prune_cat_path = Path.cwd().parent / "data/pruned_experiment/pca_pruning_feats_retained_per_cat.csv"
        #num_common_cat = 2 # at least how many categories out of 8 must have retained a feature
        #print(prune_cat_path)
        m_original = prune_m(m_original, prune_cat_path, num_common_cat)
    else:
        pass
    print(m_original.shape)
    num_feats = m_original.shape[1]
    ...
    best_score_before = 0
    best_score_after = 0
    for proj_size in [15, 30, 45]:
        for kc_size in [10_000, 20_000, 25_000]:
            for wta in [20, 22, 25]:

                #Running PCA with different numbers of components
                for k in [None, 2, int(num_feats * 0.75)]:#, int(num_feats / 2), int(num_feats * 0.75)]:
                    if k != None:
